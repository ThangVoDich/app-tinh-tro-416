...
function showSavedInvoiceImage(dataUrl, fileName = "hoa-don-tien-tro.png") {
...
function saveAndShowInvoiceImage(invoiceOverride = null) {
...
function createInvoiceImageDataUrl(invoice) {
  const room = invoice.roomSnapshot || {};
  ...
  const width = 1080;
  const rowHeight = 62;
  const height = 390 + rows.length * rowHeight + (invoice.note ? 70 : 0) + 300;
  ...
  ctx.strokeStyle = line;
  ctx.lineWidth = 2;
  ctx.fillRect(margin, y + 10, width - margin * 2, 78);
  ctx.strokeRect(margin, y + 10, width - margin * 2, 78);

  ctx.fillStyle = primary;
  ctx.font = "900 28px Arial";
  ctx.textAlign = "center";
  ctx.fillText("VUI LÒNG THANH TOÁN ĐÚNG HẠN", width / 2, y + 58);

  ctx.fillStyle = muted;
  ctx.font = "500 18px Arial";
  ctx.fillText("Created by ThangVoDich", width / 2, y + 126);

  return canvas.toDataURL("image/png");
}

function showInvoiceImage(invoiceOverride = null) {
  const invoice = isRealInvoiceObject(invoiceOverride) ? invoiceOverride : buildInvoiceData();
...
function saveMonthlyAndShowImage(card) {
...
function setupEvents() {
  ...
  $("#makeImageBtn").addEventListener("click", () => saveAndShowInvoiceImage());
