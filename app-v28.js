...
function loadImageAsync(src, timeoutMs = 12000) {
...
async function tryLoadQrImage(invoice) {
  const src = getVietQrUrl(invoice);

  try {
    return await loadImageAsync(src, 12000);
  } catch (error) {
    console.warn("Lần 1 tải QR lỗi, thử lại...", error);
  }

  await delay(800);

  try {
    return await loadImageAsync(src, 8000);
  } catch (error) {
    console.warn("Lần 2 tải QR vẫn lỗi.", error);
    return null;
  }
}
...
async function createInvoiceImageDataUrl(invoice, preloadedQrImg = null) {
  const room = invoice.roomSnapshot || {};
  ...
  try {
    const qrImg = preloadedQrImg || await tryLoadQrImage(invoice);
    if (!qrImg) throw new Error("QR không tải được");
    ...
    ctx.textAlign = "center";
    ctx.fillText("KHÔNG TẢI ĐƯỢC QR TỪ VIETQR", width / 2, qrTop + 145);
...
async function showInvoiceImage(invoiceOverride = null) {
  ...
  const preloadedQrImg = await tryLoadQrImage(invoice);
  const dataUrl = await createInvoiceImageDataUrl(invoice, preloadedQrImg);
  const room = invoice.roomSnapshot || {};
